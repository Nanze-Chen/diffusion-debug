    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        if args.class_cond:
            if args.label != "-1":
                ...
                if len(labels) == 1:
                    label = int(labels[0].strip())
                    classes = th.tensor([label]*args.batch_size, dtype=th.int, device=dist_util.dev())
                    # classes = th.randint(
                    #     low=label, high=label+1, size=(args.batch_size,), device=dist_util.dev()
                    # )
                else:
                    label_list = [int(label.strip()) for label in labels]
                    classes = th.tensor([label_list]*args.batch_size, dtype=th.int, device=dist_util.dev())
            else:
                classes = th.randint(
        ...
        sample = sample_fn(
            model,
            (args.batch_size, args.img_channels_num, args.image_size, args.image_size),
